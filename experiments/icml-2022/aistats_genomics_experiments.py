...
def run_14Cancergenomics_Full(dir_path='results/',data_path='data/', admm_iters = 5, gauss_coef=2.0, neighbors=None, rho = 1, weights='Gaussian', penalty_list = np.concatenate((np.repeat(np.inf,10),np.exp(np.linspace(-5,500,150))[::-1]),axis=0), parallel=True, output_file='NaN'):
    ...
    print(save_path)
    A, U, S, V = pcmf_full(X_c, penalty_list, rho=rho, numba=True, admm_iters = admm_iters, weights=weights, gauss_coef=gauss_coef, neighbors=neighbors, print_progress=True, parallel=parallel, output_file=save_path+'.txt')
    toc = time.time() - tic
...
def run_14Cancergenomics_Approx_uV(dir_path='results/',data_path='data/', admm_iters = 5, gauss_coef=2.0, neighbors=None, rho = 1, weights='Gaussian', penalty_list = np.concatenate((np.repeat(np.inf,10),np.exp(np.linspace(-5,500,150))[::-1]),axis=0), parallel=True, output_file='NaN'):
    ...
    # output_file = dir_path+pcmf_type+'_14Cancer_genomics_run_gausscoef'+str(gauss_coef)+'_neighbors'+str(neighbors)+'_rho'+str(rho)+'.txt'
    print(save_path)
    V, U, S = pcmf_approx_uV(X_c, penalty_list, rho=rho, numba=True, admm_iters = admm_iters, weights=weights, gauss_coef=gauss_coef, neighbors=neighbors, print_progress=True, parallel=parallel, output_file=save_path+'.txt')
    toc = time.time() - tic
    np.savez(save_path+'.npz', pcmf_type=pcmf_type, X_c=X_c, true_clusters=true_clusters, labels_names=labels_names, V=V, U=U, S=S, runtime=toc, penalty_list=penalty_list, rho=rho, numba=True, admm_iters = admm_iters, weights=weights, gauss_coef=gauss_coef, neighbors=neighbors, scale_data=True, intercept=True) 
...
def run_NCIgenomics_Approx_uV(dir_path='results/',data_path='data/', admm_iters = 5, gauss_coef=2.0, neighbors=None, rho = 1, weights='Gaussian', penalty_list = np.concatenate((np.repeat(np.inf,10),np.exp(np.linspace(-5,500,150))[::-1]),axis=0), parallel=True, output_file='NaN'):
    ...
    V, U, S = pcmf_approx_uV(X_c, penalty_list, rho=rho, numba=True, admm_iters = admm_iters, weights=weights, gauss_coef=gauss_coef, neighbors=neighbors, print_progress=True, parallel=parallel, output_file=save_path+'.txt')
    toc = time.time() - tic
    np.savez(save_path+'.npz', pcmf_type=pcmf_type, X_c=X_c, true_clusters=true_clusters, V=V, U=U, S=S, runtime=toc, penalty_list=penalty_list, rho=rho, numba=True, admm_iters = admm_iters, weights=weights, gauss_coef=gauss_coef, neighbors=neighbors, scale_data=True, intercept=True) 
